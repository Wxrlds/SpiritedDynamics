...
for log, data in log_to_plank {
   val planks = BracketHandlers.getItem(data["item"]);
	...
	craftingTable.addShaped("unify/woodslabs/" + data["name"] + "/planks", planks * 20, [
		[log, air, log],
		[air, log, air],
		[log, air, log]
	]);
...
for log, data in log_to_wood {
   val wood = BracketHandlers.getItem(data["item"]);
	...
	craftingTable.addShaped("unify/woodslabs/" + data["name"] + "/wood", wood * 8, [
		[log, log, log],
		[log, log, log],
		[log, log, log]
	]);
...
for stripped_log, data in stripped_log_to_stripped_wood {
   val stripped_wood = BracketHandlers.getItem(data["item"]);
	...
	craftingTable.addShaped("unify/woodslabs/" + data["name"] + "/stripped_wood", stripped_wood * 8, [
...
craftingTable.addShaped("unify/woodslabs/ancient/planks", <item:naturesaura:ancient_planks> * 10, [
...
craftingTable.addShaped("unify/woodslabs/ancient/wood", <item:naturesaura:ancient_bark> * 8, [
...
for slab, data in slab_to_block {
   val block = BracketHandlers.getItem(data["item"]);
	craftingTable.addShapeless("unify/woodslabs/slabtoblock/" + data["name"], block,
		[slab, slab]
	);
}