onEvent("entity.spawned", (event) => {
  if ("minecraft:lightning_bolt" == event.entity.type) {
    let entities = event.world.getEntitiesWithin(
      AABB.of(
        event.entity.x - 3,
        event.entity.y - 2,
        event.entity.z - 3,
    ...
    for (i in entities) {
      ...
      if (e.item && items[e.item.id]) {
        items[e.item.id] = items[e.item.id] + e.getItem().count;
        e.kill();
      }
    }
    ...
    if (items["appliedenergistics2:certus_quartz_crystal"]) {
      ...
      event.server.scheduleInTicks(4, event, (event) => {
        ...
        event.server.runCommandSilent(
          `summon appliedenergistics2:charged_quartz ${strikePos.x} ${strikePos.y + 0.1} ${strikePos.z} {PickupDelay:80,Invulnerable:1b,NoGravity:1b,Motion:[${0.025 - Math.random() / 20},${Math.random() / 30 + 0.01},${0.025 - Math.random() / 20}],Item:{id:"appliedenergistics2:charged_certus_quartz_crystal",Count:${output}b}}`,
        );
      });
    }
  }
